...
启动 UI 后在浏览器打开 `http://127.0.0.1:8765`，以上 CLI 操作均可通过 Web 界面完成。

### 纯 UI 操作（无需 CLI）

启动 `python -m aqt ui` 后，所有功能均可在浏览器中完成：

- **数据管理**：点击"初始化数据"一键拉取当日行情 + 历史 K 线 + 基准 + 行业分类；日常点击"拉取每日数据"增量追加；"拉取历史数据"补充回溯天数
- **回测**：填写策略参数后点击"运行回测"，结果在面板中直接展示（KPI 卡片、净值曲线、交易流水、指标明细）
- **参数扫描**：勾选"参数扫描"后设置枚举范围，自动网格搜索最优参数并排名
- **交易计划**：点击"生成交易计划"输出次日调仓建议，可指定计划日期
- **持仓模板**：点击"生成持仓模板"创建标准持仓 CSV 供编辑后导入
- **K 线图**：输入股票代码即可查看日/周/月 K 线，支持自选股管理、风险评分、买入建议
...
### 策略与风控
侧边栏策略表单支持完整参数配置：
- **策略类型**：多因子轮动 / 均线交叉 / 通道突破 / 均值回归
- **仓位管理**：持仓数、单票上限、现金缓冲、最低成交额、调仓频率
- **风控**：止损、止盈、**移动止损**、单行业上限、黑名单
- **优化**：勾选"参数扫描"展开网格搜索（枚举 Top-N、回看天数、排名指标）
...
`fetch` 命令从公开行情接口拉取当日 A 股全量数据，生成与样例数据相同格式的 CSV。也可通过 Web UI 按钮完成（初始化 / 每日拉取 / 历史补充）。
...
多因子策略支持完整风控约束，CLI 和 Web UI 均可配置：

```powershell
# 每行业最多 2 只；排除指定股票；5% 移动止损
python -m aqt backtest --data-dir data/live --start 2024-01-01 --end 2025-01-01 \
    --max-per-industry 2 --blacklist 600008,000002 --trailing-stop 5
```
...
| 止损 (%) | `--stop-loss` | 策略表单 |
| 止盈 (%) | `--take-profit` | 策略表单 |
| 移动止损 (%) | `--trailing-stop` | 策略表单 |
| 单行业上限 | `--max-per-industry` | 策略表单 |
| 黑名单 | `--blacklist` | 策略表单（逗号分隔） |

行业分类数据通过 `fetch --init`（或 UI "初始化数据"按钮）自动从 akshare 拉取并保存为 `industry.csv`，后续回测自动加载。
